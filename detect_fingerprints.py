...
parser.add_argument('--w_std', type=float, default=0.)
parser.add_argument('--rewatermark', action='store_true')
parser.add_argument('--rewatermark_weight', type=float, default=0.)
...
def load_encoder_decoder():
    ...
    if args.rewatermark:
        ...
        if args.cuda == '-1':
            EncoderNet.load_state_dict(torch.load(args.rewatermark_encoderpath, map_location='cpu'))
        else:
...
def save_samples(): 
    ...
    def round_weights(m):
        if type(m) == nn.Conv2d:
            m.weight.data = torch.round(m.weight * 10**args.w_n_digits) / (10**args.w_n_digits)
            m.bias.data = torch.round(m.bias * 10**args.w_n_digits) / (10**args.w_n_digits)
...
def extract_secret():
    ...
    if args.rewatermark:
        ...
        fake_images = fake_images * (1 - args.rewatermark_weight) + args.rewatermark_weight * (2 * EncoderNet(secret_batch, normalized_fake) - 1)
    ...
    if len(revealed.size()) == 2:
        ...
        import numpy as np
        idx = np.argsort((secret_batch.detach() == rev_secret).float().mean(dim=1).cpu().detach().numpy())
