...
def generate_images(network_pkl, seeds, watermark_seeds, batch_size, truncation_psi):
    output_txt = dnnlib.make_run_dir_path('watermarks.txt')
    f = open(output_txt, 'w')

    print('Loading networks from "%s"...' % network_pkl)
    _G, _D, Gs = pretrained_networks.load_networks(network_pkl, load_decoder=False)
    ...
    for seed_idx, seed in tqdm(enumerate(seeds)):
        ...
        z = rnd.randn(batch_size, *Gs.input_shape[1:]) # [minibatch, component]
        tflib.set_vars({var: rnd.randn(*var.shape.as_list()) for var in noise_vars}) # [height, width]

        grid_size = (1, 1)
        
        images = Gs.run(z, None, is_validation=True, minibatch_size=1, **Gs_kwargs) # [minibatch, height, width, channel]
        generated_batches.append(torch.from_numpy(images))

        for i in range(batch_size):
            image_save_path = dnnlib.make_run_dir_path('seed{:04d}_{}.png'.format(seed, i))
            PIL.Image.fromarray(images[i], 'RGB').save(image_save_path)

 
    tensor_save_path = dnnlib.make_run_dir_path('images_tensor.pth'.format(seed, i)) 
    torch.save(torch.cat(generated_batches, dim=0).float().permute(0, 3, 1, 2)/255, tensor_save_path) 
...
Run 'python %(prog)s <subcommand> --help' for subcommand help.''',
    ...
    parser_generate_images.add_argument('--watermark_seeds', type=_parse_num_range, help='List of random seeds for watermarks', required=True)
    parser_generate_images.add_argument('--batch_size', type=int, help='Batch size', required=True)
