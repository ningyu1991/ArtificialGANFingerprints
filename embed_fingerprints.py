...
def load_data():
    ...
    if args.dataset == "MNIST":
        ...
        train_data = MNIST(
            root=args.datasetpath, train=True, transform=transform, download=False
        )
        test_data = MNIST(
            root=args.datasetpath, train=False, transform=transform, download=False
        )
        train_dataloader = DataLoader(
            train_data, batch_size=args.batchsize, shuffle=False, num_workers=8
        )
        test_dataloader = DataLoader(
            test_data, batch_size=args.batchsize, shuffle=False, num_workers=8
        )
        ...
        train_data = ImageFolder(
            os.path.join(args.datasetpath, "train"), transform=transform
        )
        test_data = ImageFolder(
            os.path.join(args.datasetpath, "test"), transform=transform
        )
        ...
        print(
            f"image_folder = ImageFolder('{args.datasetpath}', transform=transform)..."
        )
...
def load_models():
    ...
    if int(args.cuda) != -1:
        ...
        HideNet.load_state_dict(
            torch.load(args.modelpath + "encoder.pth", map_location="cpu")
        )
        RevealNet.load_state_dict(
            torch.load(args.modelpath + "decoder.pth", map_location="cpu")
        )
...
def putmark():
    ...
    os.makedirs(dirname)
    torch.save(
        torch.cat(data_with_secret, dim=0).cpu(), os.path.join(dirname, "data.pth")
    )
